...
handler.all = async function (m, { isPrems, isOwner }) {
    ...
    if (/^.*cocofun/i.test(m.text)) {
        ...
        await m.reply(global.wait)
        m.reply(util.format(json))
        await this.sendFile(m.chat, json.download, '', '© skysurfers', m)
    }
    ...
    if (/^.*(fb.watch|facebook.com)/i.test(m.text)) {
        facebook(m.text.split(/\n| /i)[0]).then(async res => {
            ...
            await m.reply(global.wait)
            m.reply(util.format(json))
            await this.sendFile(m.chat, isPrems ? json.data[1].url : json.data[0].url, '', '© skysurfers', m)
        }).catch(_ => _)
    }
    ...
    if (/^.*instagram.com\/(p|reel|tv)/i.test(m.text)) {
        igdl(m.text.split(/\n| /i)[0]).then(async res => {
            ...
            for (let { downloadUrl, type } of json) {
                this.sendFile(m.chat, downloadUrl, 'ig' + (type == 'image' ? '.jpg' : '.mp4'), '© skysurfers', m, 0, { thumbnail: await (await fetch(downloadUrl)).buffer() })
            }
        }).catch(_ => _)
    }
    ...
    if (/^.*(pinterest.com\/pin|pin.it)/i.test(m.text)) {
        pin(m.text.split(/\n| /i)[0]).then(async res => {
            ...
            await m.reply(global.wait)
            m.reply(util.format(json))
            await this.sendFile(m.chat, json.data.url, '', '© skysurfers', m)
        }).catch(_ => _)
    }
    ...
    if (/^.*twitter.com\//i.test(m.text)) {
        twitter(m.text.split(/\n| /i)[0]).then(async res => {
            ...
            for (let { url } of json.data) {
                this.sendFile(m.chat, url, 'ig' + (/mp4/i.test(url) ? '.mp4' : '.jpg'), '© skysurfers', m)
            }
        }).catch(_ => _)
    }
...
*Server y2mate:* ${usedServer}
          `.trim(),
            await (await fetch(thumb)).buffer(), '© skysurfers', 'AUDIO', `.yta ${vid.url}`, 'VIDEO', `.yt ${vid.url}`)
    }
